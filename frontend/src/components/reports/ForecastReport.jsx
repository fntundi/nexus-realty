...
export default function ForecastReport({ data, isLoading }) {
  ...
  const exportToPDF = (reportData) => {
    ...
    doc.setFontSize(18);
    doc.text('Closing Forecast Report', 14, yPos);
    yPos += 10;
    ...
    const colWidths = [50, 35, 20, 25, 30, 20];
    const startX = 14;
    
    doc.setFont(undefined, 'bold');
    doc.setFontSize(9);
    let x = startX;
    ['Property', 'Agent', 'Stage', 'Value', 'Close Date', 'Prob'].forEach((header, i) => {
    ...
    reportData.data.slice(0, 20).forEach(t => {
      ...
      x += colWidths[3];
      doc.text(format(new Date(t.closing_date), 'MMM d'), x, yPos);
      x += colWidths[4];
